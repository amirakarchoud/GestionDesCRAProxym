...
server.post(
  ...
  async (req, res) => {
    const { message, emailList } = req.body; // Retrieve the message and email list from the request body

    for (const target of await notificationApp.notification.installations()) {
      if (target.type === "Channel") {
        const members = await target.members();
        for (const member of members) {
          if (member.account.email && emailList.includes(member.account.email)) {
